...
export default function Profile({ user, onUpdate }: Props) {
  ...
  useEffect(() => {
    fetch(api(`/api/app/history/${user.id}`).then(r => r.json()).then(d => {
  ...
  const save = async () => {
    ...
    const res = await fetch(api('/api/auth/update-profile', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id, name, phone, avatar }) })
