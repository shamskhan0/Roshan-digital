...
export default function AdminPanel({ user, onNav }: Props) {
  ...
  const load = async () => {
    try {
      const [s, u, d, w, p, t, st] = await Promise.all([
        fetch('/api/app/admin/stats').then(r => r.json()), fetch('/api/app/admin/users').then(r => r.json()),
        fetch('/api/app/admin/deposits').then(r => r.json()), fetch('/api/app/admin/withdrawals').then(r => r.json()),
        fetch('/api/app/admin/plans').then(r => r.json()), fetch('/api/app/admin/tasks').then(r => r.json()),
        fetch('/api/app/admin/settings').then(r => r.json()),
      ])
  ...
  const approveDeposit = async (id: string) => withProcessing(id, async () => { await fetch(`/api/app/admin/deposits/${id}/approve`, { method: 'POST' }); load() })
  const rejectDeposit = async (id: string) => { const reason = prompt('Rejection reason:'); await withProcessing(id, async () => { await fetch(`/api/app/admin/deposits/${id}/reject`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reason }) }); load() }) }
  const approveWithdrawal = async (id: string) => withProcessing(id, async () => { await fetch(`/api/app/admin/withdrawals/${id}/approve`, { method: 'POST' }); load() })
  const rejectWithdrawal = async (id: string) => { const reason = prompt('Rejection reason:'); await withProcessing(id, async () => { await fetch(`/api/app/admin/withdrawals/${id}/reject`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reason }) }); load() }) }
  const makeAdmin = async (id: string) => withProcessing(id, async () => { await fetch(`/api/app/admin/users/${id}/role`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ role: 'admin' }) }); load() })
  const toggleActive = async (id: string) => withProcessing(id, async () => { await fetch(`/api/app/admin/users/${id}/toggle-active`, { method: 'POST' }); load() })
  const forceLogout = async (id: string) => withProcessing(id, async () => { await fetch('/api/app/admin/force-logout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: id }) }); load() })
  const saveSettings = async () => { await fetch('/api/app/admin/settings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(settings) }); load() }
  ...
  const deletePlan = async (id: string) => { await withProcessing(id, async () => { await fetch(`/api/app/admin/plans/${id}`, { method: 'DELETE' }); load() }) }
  ...
  const deleteTask = async (id: string) => { await withProcessing(id, async () => { await fetch(`/api/app/admin/tasks/${id}`, { method: 'DELETE' }); load() }) }
  const broadcast = async () => { if (!broadcastMsg) return; await fetch('/api/app/admin/broadcast', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: '📢 Announcement', message: broadcastMsg, type: 'info' }) }); setBroadcastMsg(''); load() }
