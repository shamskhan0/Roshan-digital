...
export default function Withdraw({ user }: Props) {
  ...
  useEffect(() => {
    fetch(`/api/app/wallet/${user.id}`).then(r => r.json()).then(d => setWallet(d.wallet)).catch(() => {})
    fetch(`/api/app/withdrawals/${user.id}`).then(r => r.json()).then(d => setWithdrawals(d.withdrawals || [])).catch(() => {})
  }, [user.id])

  const submit = async () => {
    if (!amount || !method || !accountNumber) { setMsg('All fields are required'); return }
    setLoading(true); setMsg('')
    const res = await fetch('/api/app/withdrawals', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id, amount: parseFloat(amount), method, accountName, accountNumber }) })
    const data = await res.json()
    if (data.ok) { setMsg(data.message); setAmount(''); setMethod(''); setAccountName(''); setAccountNumber(''); const w = await fetch(`/api/app/wallet/${user.id}`).then(r => r.json()); setWallet(w.wallet); const wl = await fetch(`/api/app/withdrawals/${user.id}`).then(r => r.json()); setWithdrawals(wl.withdrawals || []); setTab('history') }
