...
export default function Investment({ user }: Props) {
  ...
  useEffect(() => {
    Promise.all([
      fetch(api('/api/app/investment-plans').then(r => r.json()),
      fetch(api(`/api/app/investments/${user.id}`).then(r => r.json()),
      fetch(api(`/api/app/wallet/${user.id}`).then(r => r.json()),
    ]).then(([p, i, w]) => {
      setPlans(p.plans || [])
  ...
  const activate = async (planId: string) => {
    ...
    const res = await fetch(api('/api/app/investments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id, planId }) })
