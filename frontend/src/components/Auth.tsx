...
export default function Auth({ onLogin }: AuthProps) {
  ...
  const sendOTP = async (target: string, purpose: string) => {
    const res = await fetch(api('/api/otp/send'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ target, purpose }) })
  ...
  const handleRegister = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(api('/api/auth/register'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password, name, phone, referralCode: referralCode || undefined }) })
      const data = await res.json()
  ...
  const handleVerifySignup = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(api('/api/otp/verify'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ target: email, code: otp, purpose: 'signup' }) })
      const data = await res.json()
      if (!res.ok) { setError(data.error || 'Invalid OTP'); return }
      await fetch(api('/api/auth/verify-account'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: pendingUser.id }) })
      await fetch(api('/api/auto-seed'))
      onLogin(pendingUser)
  ...
  const handleVerifyLogin = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(api('/api/otp/verify'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ target: email, code: otp, purpose: 'login' }) })
      const data = await res.json()
      if (!res.ok) { setError(data.error || 'Invalid OTP'); return }
      await fetch(api('/api/auto-seed'))
      onLogin(pendingUser)
  ...
  const handleResetPassword = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(api('/api/auth/reset-password'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, code: otp, newPassword }) })
      const data = await res.json()
      if (!res.ok) { setError(data.error || 'Error'); return }
