...
export default function Community({ user }: Props) {
  ...
  const load = (p: number) => {
    fetch('/api/app/community/posts?page=' + p + '&limit=20')
      .then(r => r.json())
      .then(d => {
  ...
  const createPost = async () => {
    ...
    setPosting(true)
    const res = await fetch('/api/app/community/posts', {
      method: 'POST',
  ...
  const deletePost = async (id: string) => {
    ...
    await fetch('/api/app/community/posts/' + id, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
  ...
  const toggleLike = async (postId: string) => {
    await fetch('/api/app/community/posts/' + postId + '/like', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const addComment = async (postId: string) => {
    ...
    await fetch('/api/app/community/posts/' + postId + '/comment', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const deleteComment = async (id: string) => {
    await fetch('/api/app/community/comments/' + id, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userId: user.id, isAdmin: user.role === 'admin' }),
    })
